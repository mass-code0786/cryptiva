...
import { approveAdminWithdrawal, fetchAdminWithdrawals, type AdminWithdrawalItem } from "../../services/adminService";

const getUser = (user: AdminWithdrawalItem["userId"]) => ({
  name: user?.name || "User",
  email: user?.email || "-",
  userId: user?.userId || "-",
});
  ...
  return (
    <section className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-300">Approve pending withdrawals after manual transfer verification.</p>
      </div>
      ...
      <div className="space-y-3">
        {loading && (
          <p className="rounded-xl border border-cyan-700/25 bg-slate-950/45 px-3 py-3 text-sm text-slate-300">
            Loading withdrawals...
          </p>
        )}
